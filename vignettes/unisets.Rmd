...
- The required `"element"` and `"set"`columns are substitued by the index of the corresponding identifier in the `elementData` and `setData` slot, to create the `from` and `to` slots of a `Hits` object, respectively.
    If not supplied, the `elementData` and `setData` slots are automatically constructed from unique values found in the `"element"` and `"set"`columns.
...
Conveniently, the `as` method can be used to obtain relations and associated metadata as a `DataFrame` substituting hits for their corresponding element and set identifiers.
