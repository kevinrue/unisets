...
It is useful to note that this method of converting Gene Ontology maps to `r Githubpkg("unisets")` objects automatically fetches metadata for each GO identifier from the `GO.db` package.
The metadata is stored it in the `setData` slot of the object returned.
This metadata can be accessed using the accessor method of the same name.

```{r}
setData(base_sets)[, c("ONTOLOGY", "TERM")]
```
...
Below, we use the `nRelationsn` method to show the number of relations between genes and GO terms imported from the `org.Hs.egGO2ALLEGS` map.

```{r message=FALSE}
base_sets <- as(org.Hs.egGO2ALLEGS, "BaseSets")
format(nRelations(base_sets), big.mark=",")
```
