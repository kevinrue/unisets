...
To import or export a GMT file, one can use the generic `import` and `export` methods, which autodetect the file format using the file extension (.gmt). These methods work with objects of the `BaseSets` class. Note that the .gmt extension is required for both cases.

When using `import`, a `BaseSets` class object is produced
